...
if [[ -z $endpoint ]]; then
  log "bootstrapping the agent network against proxy cluster agent-network-k8s.example.com"
  for i in $(seq 1 30); do
    out=$(curl -sS -o /tmp/.an-settings -w '%{http_code}' -X POST "$API/agent-network/settings" \
      ...
      --data "$(jq -nc --arg p "agent-network-k8s.example.com" '{proxy_address:$p}')" || true)
...
if [[ $observed_proxy != "agent-network-k8s.example.com" ]]; then
  log "FATAL: settings carry proxy_address $observed_proxy, not this deployment's agent-network-k8s.example.com"
  exit 1
fi
